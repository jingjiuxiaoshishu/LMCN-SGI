...
class Sat_selector_CSGI(Scheduler_node):
    ...
    def update_gsl(self,min_RST,type = "change"):
        ...
        count_shift = {}
        shift_index_to_pre_sat = []
        if type == "change":
            for src_gid in range(self.num_gs):
                shift_index_to_pre_sat.append([])
            ...
            for src_gid in range(self.num_gs):
                if ( most_x_shift, most_y_shift) in shift_index_to_pre_sat[src_gid]:
                    x = (most_x_shift + self.gsls_index[src_gid][0] + self.num_orbs) % self.num_orbs
                    y = (most_y_shift + self.gsls_index[src_gid][1] + self.num_sats_per_orbs) % self.num_sats_per_orbs
    ...
    def process(self):
        ...
        if need_init:
            ...
            for gid,gsl in self.gsls.items():
                if self.get_visible_time_left(gsl,gid,current_time) <= 0:
                    print(f"change {self.curr_slot},{gid},{gsl}",self.get_visible_time_left(gsl,gid,current_time))
...
if __name__ == "__main__":
    ...
    sat_selector_CSGI = Sat_selector_CSGI(visible_times, satellites, num_orbs, num_sats_per_orbs, shift_between_last_and_first, plus_grid_graph, epoch, time_step, sim_duration)
    # for gid,gsl in sat_selector_CSGI.gsls.items():
    #     # 对每个 gs 的 gsl 更新两次
    #     for i in range(2):
    #         sat_selector_CSGI.add_gsl_update_event(gid,i,0)
    gsls_of_all_case = []
    switch = []
    selects = []
    for i in range(sat_selector_CSGI.num_time_slot):
        print(i)
        sat_selector_CSGI.process()
        if (not gsls_of_all_case) or gsls_of_all_case[-1]!=sat_selector_CSGI.gsls:
            gsls_of_all_case.append(copy.deepcopy(sat_selector_CSGI.gsls))
            switch.append(i)
            selects.append(sat_selector_CSGI.selects)

    print(gsls_of_all_case)
