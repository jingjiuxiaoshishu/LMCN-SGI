...
# 且网络在多条路由代价相近的动态路径之间频繁切换，造成较高的路由流失率。
# 因此，对于地面站而言，我们结合路由的历史状态，修改了GSP的路由更新规则，
# 延长单条 GSL 的使用时间，提高链路使用率，从而有效解决了地面站的星地切换频繁问题。
# basic process
# 1. 计算从卫星到地面站的最短路径：
#     - 首先，为每个卫星到每个地面站的对初始化一个到无穷大的距离。
#     - 检查前一个转发状态，如果存在并且之前选择的路径仍然可用，则使用它。
#     - 否则，查找当前卫星与目的地面站之间的最短路径。
#     - 最后，将新的或已更改的转发状态写入文件，并更新转发状态字典。

# 2. 计算从地面站到地面站的最短路径：
#     - 针对每对不同的地面站，查找最佳的中继卫星。
#     - 使用前一个转发状态作为候选中继（如果存在并且仍然有效）。
#     - 否则，基于其与目的地面站的距离选择最佳的源地面站可见卫星。
#     - 与之前类似，更新转发状态。

# 3. 返回转发状态：函数结束时返回转发状态
